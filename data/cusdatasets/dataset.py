...
class Dataset(data.Dataset):
    ...
    def __init__(self, txt_path, base_path, main_transform=None, img_transform=None, train=True, datasetname='Empty', frame_intervals=(3,6), model="GD3A"):
        ...
        for scene_name in scene_names:
            ...
            elif datasetname == 'WuhanMetroCrowd':
                img_path, label = WuhanMetro_ImgPath_and_Target(base_path, scene_name.strip())
    ...
    def __getitem__(self, index):
        ...
        max_interval = min(max(1, self.scene_id[scene_name] // 2), self.frame_intervals[1])
        min_interval = min(self.frame_intervals[0], max_interval)
        tmp_intervals = random.randint(min_interval, max_interval)
...
def WuhanMetro_ImgPath_and_Target(base_path, scene_name):
    img_path = []
    labels = []
    scene_img_root = os.path.join(base_path, "frames", scene_name)
    ann_root = os.path.join(base_path, "annotations", scene_name)
    img_ids = os.listdir(scene_img_root)
    img_ids = sorted(img_ids, key=lambda x: int(x.split('.')[0]))

    for img_id in img_ids:
        frame_id = int(img_id.split('.')[0])
        single_path = os.path.join(scene_img_root, img_id)
        gt_path = os.path.join(ann_root, img_id.replace(".jpg", ".json"))
        ...
        for item in points_data:
            ...
            if label not in {"pedestrian", "inflow", "outflow", "both"}:
                continue
            points.append([float(item["x"]), float(item["y"])])
            inflow.append(1 if label in {"inflow", "both"} else 0)
            outflow.append(1 if label in {"outflow", "both"} else 0)

        if points:
            points_tensor = torch.tensor(points, dtype=torch.float32).view(-1, 2)
            inflow_tensor = torch.tensor(inflow, dtype=torch.long)
            outflow_tensor = torch.tensor(outflow, dtype=torch.long)
        else:
            points_tensor = torch.empty((0, 2), dtype=torch.float32)
            inflow_tensor = torch.empty((0,), dtype=torch.long)
            outflow_tensor = torch.empty((0,), dtype=torch.long)

        img_path.append(single_path)
        labels.append({
            'scene_name': scene_name,
            'frame': frame_id,
            'inflow': inflow_tensor,
            'outflow': outflow_tensor,
    ...
    def __init__(self, scene_name, base_path, main_transform=None, img_transform=None, interval=1, skip_flag=True, target=True, datasetname='Empty'):
        ...
        if self.target:
            if datasetname == 'SENSE':
                ...
                self.imgs_path, self.label = WuhanMetro_ImgPath_and_Target(self.base_path, scene_name)
            elif 'MovingDroneCrowd' in datasetname:
            ...
            if datasetname == 'WuhanMetroCrowd':
                self.imgs_path = WuhanMetro_ImgPath(self.base_path, scene_name)
            elif 'MovingDroneCrowd' in datasetname:
...
def WuhanMetro_ImgPath(base_path, scene_name):
    img_path = []
    root = osp.join(base_path, 'frames', scene_name)
    img_ids = os.listdir(root)
    img_ids = sorted(img_ids, key=lambda x: int(x.split('.')[0]))
