...
class Video_Counter(nn.Module):
    ...
    def SDNet_forward(self, img, target):
        ...
        physical_loss_weight = getattr(self.cfg, 'PHYSICAL_LOSS_WEIGHT', 0.0)
        temporal_count_loss_weight = getattr(self.cfg, 'TEMPORAL_COUNT_LOSS_WEIGHT', 0.0)
        flow_balance_loss_weight = getattr(self.cfg, 'FLOW_BALANCE_LOSS_WEIGHT', 0.0)
        share_consistency_weight = getattr(self.cfg, 'SHARE_CONSISTENCY_LOSS_WEIGHT', 0.0)
        if physical_loss_weight > 0 or temporal_count_loss_weight > 0 or flow_balance_loss_weight > 0 or share_consistency_weight > 0:
            pred_global_cnt = pre_global_den.flatten(1).sum(dim=1) / self.cfg_data.DEN_FACTOR
            pred_in_out_cnt = pre_in_out_den.flatten(1).sum(dim=1) / self.cfg_data.DEN_FACTOR
            if physical_loss_weight > 0:
                prev_cnt = pred_global_cnt[0::2]
                next_cnt = pred_global_cnt[1::2]
                out_cnt = pred_in_out_cnt[0::2]
                in_cnt = pred_in_out_cnt[1::2]
                conserved_next_cnt = prev_cnt - out_cnt + in_cnt
                all_loss['physical'] = F.smooth_l1_loss(next_cnt, conserved_next_cnt) * physical_loss_weight
            if temporal_count_loss_weight > 0 or flow_balance_loss_weight > 0:
                gt_global_cnt = torch.stack([
                    data['points'].new_tensor(float(data['points'].shape[0]))
                    for data in target
                ])
                gt_delta = gt_global_cnt[1::2] - gt_global_cnt[0::2]
                pred_delta = pred_global_cnt[1::2] - pred_global_cnt[0::2]
                if temporal_count_loss_weight > 0:
                    all_loss['temporal_count'] = F.smooth_l1_loss(pred_delta, gt_delta) * temporal_count_loss_weight
                if flow_balance_loss_weight > 0:
                    pred_flow_delta = pred_in_out_cnt[1::2] - pred_in_out_cnt[0::2]
                    all_loss['flow_balance'] = F.smooth_l1_loss(pred_flow_delta, gt_delta) * flow_balance_loss_weight
            if share_consistency_weight > 0:
                pred_share_cnt = pre_share_den.flatten(1).sum(dim=1) / self.cfg_data.DEN_FACTOR
                all_loss['share_consistency'] = F.smooth_l1_loss(
                    pred_share_cnt[0::2],
                    pred_share_cnt[1::2],
                ) * share_consistency_weight
