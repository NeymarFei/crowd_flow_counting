...
def param_groups_weight_decay(model, weight_decay):
    if optim_factory is not None:
        return optim_factory.param_groups_weight_decay(model, weight_decay)
    decay, no_decay = [], []
...
class Trainer():
    def __init__(self, cfg_data, pwd):
        ...
        if cfg.MODEL == "GD3A":
            ...
            param_groups = param_groups_weight_decay(self.model_without_ddp, cfg.WEIGHT_DECAY)
        ...
        if cfg.MODEL == "SDNet" and cfg.PRE_TRAIN_COUNTER:
            ...
            print("Warning: cfg.PRE_TRAIN_COUNTER is empty; SDNet will train from ImageNet backbone initialization.")
        ...
        if is_main_process():
            self.writer, self.log_txt = logger(self.exp_path, self.exp_name, self.pwd, ['exp','eval','figure','img', 'vis','output', 'test_results', 'pre_train_model', 'logs', 'data'], resume=cfg.RESUME)
    ...
    def forward(self):
        for epoch in range(self.epoch, cfg.MAX_EPOCH + 1):
            ...
            print( '='*20 )
            if is_main_process():
                self.save_latest_state(next_epoch=epoch + 1)
    ...
    def save_latest_state(self, next_epoch=None):
        latest_state = {'train_record': self.train_record,
                        'net': self.model.state_dict(),
                        'optimizer': self.optimizer.state_dict(),
                        'epoch': self.epoch + 1 if next_epoch is None else next_epoch,
                        'i_tb': self.i_tb,
                ...
                if is_main_process():
                    ...
                    if cfg.MODEL == "GD3A":
                        ...
                        print('[cnt: gt: %.1f pred: %.1f max_pre: %.1f max_gt: %.1f]  ' %
                         (gt_global_cnt.item(), pre_global_cnt.item(), pre_global_den.max().item()*self.cfg_data.DEN_FACTOR, gt_global_den.max().item()*self.cfg_data.DEN_FACTOR))
                    
            if is_main_process() and getattr(cfg, 'SAVE_VIS', True) and (self.i_tb) % 100 == 0:
    ...
    def validate(self):
        ...
        scenes_pred_dict = []
        scenes_gt_dict = []
        save_vis = getattr(cfg, 'SAVE_VIS', True)
            ...
            for vi, data in enumerate(gen_tqdm, 0):
                ...
                if frame_signal == 'match':
                    ...
                    with torch.no_grad():
                        ...
                        if save_vis and vi % self.val_frame_intervals == 0:
                            img0 = img[0]
    ...
    def load_global_counter(self):
        if cfg.global_counter == 'customed':
            ...
            if Config is None or STEERER is None:
                raise ImportError("GD3A with STEERER requires mmcv and STEERER dependencies.")
