...
parser.add_argument(
    '--test_visual', type=str2bool, default=False,
    help='Whether to save test visualizations')
parser.add_argument(
    '--skip_flag', type=str2bool, default=True,
    help='if you need to caculate the MIAE and MOAE, it should be False')
parser.add_argument(
    '--test_max_long', type=int, default=None,
    help='Optional override for cfg_data.TEST_MAX_LONG, useful for quick smoke tests')
parser.add_argument(
    '--test_max_short', type=int, default=None,
    help='Optional override for cfg_data.TEST_MAX_SHORT, useful for quick smoke tests')
parser.add_argument(
    '--max_scenes', type=int, default=None,
    help='Optional scene limit for smoke tests')
parser.add_argument(
    '--max_batches', type=int, default=None,
    help='Optional per-scene batch limit for smoke tests')
parser.add_argument(
...
def test(cfg_data):
    ...
    for scene_id, (scene_name, sub_valset) in enumerate(test_loader, 0):
        if opt.max_scenes is not None and scene_id >= opt.max_scenes:
            break
        ...
        for vi, data in enumerate(gen_tqdm, 0):
            if opt.max_batches is not None and vi >= opt.max_batches:
                break
...
if __name__=='__main__':
    ...
    if opt.test_max_long is not None:
        cfg_data.TEST_MAX_LONG = opt.test_max_long
    if opt.test_max_short is not None:
        cfg_data.TEST_MAX_SHORT = opt.test_max_short
