    ...
    try:
        ...
        class SummaryWriter:
            def __init__(self, *args, **kwargs):
                print("tensorboardX is not installed; TensorBoard logging is disabled.")
...
def update_model(trainer, scores, main_meric='seq_MAE'):
    ...
    latest_state = {'train_record':train_record, 'net':trainer.model.state_dict(), 'optimizer':trainer.optimizer.state_dict(),
                    'epoch': trainer.epoch + 1, 'i_tb':trainer.i_tb,\
...
def save_visual_results(data, restor_transform, save_base, iter_num, rank, visual_count=None, mode='test'):
    ...
    if mode == 'train':
        ...
        if tifffile is None:
            print("tifffile is not installed; saving png fallback for test visualization.")
            cv2.imwrite(os.path.join(save_base, "{}_{}_visual.png".format(rank, iter_num)), cv2.cvtColor(out, cv2.COLOR_BGR2RGB))
            return
